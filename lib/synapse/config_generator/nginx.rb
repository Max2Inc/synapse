...
class Synapse::ConfigGenerator
    class Nginx < BaseGenerator
        ...
        def initialize(opts)
            puts opts
            %w{main events http}.each do |req|
        ...
        def normalize_watcher_provided_config(service_watcher_name, service_watcher_config)
            ...
            log.debug "synapse: service watcher config is #{service_watcher_config}"
            defaults = {
                'mode' => 'http',
                'upstream' => [],
                'server' => [],
        ...
        def generate_config(watchers)
            ...
            log.debug "generate_config: new_config is #{new_config}"
            ...
            watchers.each do |watcher|
                ...
                log.debug "synapse:watcher_config: #{watcher_config}"
        ...
        def generate_base_config
            ...
            @contexts.keys.select{|key| !(["main", "http", "stream", "server"].include?(key))}.each do |context|
                base_config << "#{context} {"
        ...
        def generate_server(watcher)
            watcher_config = watcher.config_for_generator[name]
            servers = (@contexts['server'] || []).collect {|option| "\t#{option}"}
        ...
        def generate_proxy(mode, upstream_name, watcher)
            backends = watcher.backends
            watcher_config = watcher.config_for_generator[name]
            log.debug "generate_proxy: #{watcher_config}"

            upstream_name = "http://#{upstream_name}" if mode == 'http'
            empty_upstream = true if backends.empty? else false
            case mode
                when 'http'
                    if empty_upstream
                        value = "\t\t\treturn 503;"
            ...
            # generate proxy blocks for the monitored services 
            # TODO: handle the case when no proxy is needed 
            # TODO: handle tcp and http separately
            location_blocks = []
            (watcher_config['location_block'] || []).each do |block|
                unless block.has_key?('location')
